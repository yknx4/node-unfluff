...
const cleanBadTags = function (doc) {
    ...
    const toRemove = doc('*').filter(function (_, e) {
        ...
        const toMatch = [tCheerio.attr('id'), tCheerio.attr('class'), tCheerio.attr('name')].filter((t) => t != null);
...
const removeNodesRegex = function (doc, pattern) {
    const toRemove = doc('div').filter(function (_, e) {
        pattern.lastIndex = 0;
        const tCheerio = doc(e);
        const toMatch = [tCheerio.attr('id'), tCheerio.attr('class')].filter((t) => t != null);
