...
suite('Unfluff', function () {
  const cleanTestingText = (text: any, origTextLength: any) =>
    text
      .replace(/\n\n/g, ' ')
      .replace(/\ \ /g, ' ')
      .slice(0, +(origTextLength - 1) + 1 || undefined)
    ...
    return _.each(fields, function (field: any) {
      if (field === 'title') {
        ...
        return eq(
          origText,
          partialExtractText,
          `${site}: cleaned text from partial extract didn't match expected value`,
        )
      } else if (field === 'link') {
        // @dummy ts-migrate(2304) FIXME: Cannot find name 'eq'.
        ...
        return eq(
          data.canonicalLink,
          dataLazy.canonicalLink(),
          `${site}: canonical link from partial extraction didn't match expected value`,
        )
      } else if (field === 'image') {
        // @dummy ts-migrate(2304) FIXME: Cannot find name 'eq'.
        ...
        return eq(
          data.description,
          dataLazy.description(),
          `${site}: description from partial extraction didn't match expected value`,
        )
      } else if (field === 'lang') {
        // @dummy ts-migrate(2304) FIXME: Cannot find name 'eq'.
        ...
        return eq(
          data.keywords,
          dataLazy.keywords(),
          `${site}: meta keywords from partial extraction didn't match expected value`,
        )
        ...
        return eq(
          data.favicon,
          dataLazy.favicon(),
          `${site}: favicon url from partial extraction didn't match expected value`,
        )
      } else if (field === 'tags') {
        ...
        return arrayEq(
          sortedTags,
          dataLazy.tags().sort(),
          `${site}: meta tags from partial extraction didn't match expected value`,
        )
      } else if (field === 'links') {
        ...
        // @dummy ts-migrate(2304) FIXME: Cannot find name 'deepEq'.
        deepEq(orig.expected.links.sort(), sortedLinks, `${site}: links didn't match expected value`)
        // @dummy ts-migrate(2304) FIXME: Cannot find name 'deepEq'.
        return deepEq(
          orig.expected.links.sort(),
          sortedLazyLinks,
          `${site}: links from partial extraction didn't match expected value`,
        )
        ...
        return deepEq(
          sortedVideos,
          dataLazy.videos().sort(),
          `${site}: videos from partial extraction didn't match expected value`,
        )
      } else {
        // Oops!
        // @dummy ts-migrate(2304) FIXME: Cannot find name 'eq'.
  ...
  // @dummy-directive ts-migrate(2582) FIXME: Cannot find name 'test'. Do you need to install ty... Remove this comment to see the full error message
  test.skip('gets cleaned text - Techcrunk', () => checkFixture('techcrunch1', ['cleaned_text']))
  ...
  // @dummy-directive ts-migrate(2582) FIXME: Cannot find name 'test'. Do you need to install ty... Remove this comment to see the full error message
  test.skip('gets cleaned text - Goose Regressions', function () {
  ...
  // @dummy-directive ts-migrate(2582) FIXME: Cannot find name 'test'. Do you need to install ty... Remove this comment to see the full error message
  test.skip('gets cleaned text - USA Today', function () {
